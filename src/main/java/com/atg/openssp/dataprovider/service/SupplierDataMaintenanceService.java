...
import com.atg.openssp.dataprovider.provider.handler.SupplierDataMaintenanceHandler;
 ...
 */
@WebServlet(value = SupplierDataMaintenanceHandler.CONTEXT, asyncSupported = false, name = "SupplierMaintenanceData-Service")
public class SupplierDataMaintenanceService extends CoreDataServlet<DataHandler> {
	...
	protected DataHandler getHandler(final HttpServletRequest request, final HttpServletResponse response) throws RequestException {
		String handlerClassName = LocalContext.getSupplierDataMaintenanceHandlerClass();
		if (handlerClassName == null) {
			return new SupplierDataMaintenanceHandler(request, response);
		} else {
			try {
